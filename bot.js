...
client.on('message', message => {
    ...
    if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('ليس لديك صلاحيات');
     ...
     client.user.setGame(`Toronto Bot | ${client.guilds.size}`)//حقوق دايموند كودز
    ...
    client.on('message',async msg => {
      ...
      client.on("message", (message) => {
        ...
        client.on ("guildMemberAdd", member => {
          ...
          var role = member.guild.roles.find ("name", "toronto-army");
          member.addRole (role);
...
client.on("message", (message) => {
  ...
  if (message.content.toLowerCase().startsWith(prefix + `bot-ping`)) {
...
client.on("guildMemberAdd", async member => {
  ...
  if(date < 14) {
    member.ban("وهمي ( اقل من اسبوعين ) انشاء الحساب .")
  }
});
